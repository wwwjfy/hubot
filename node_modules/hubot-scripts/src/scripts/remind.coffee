...
module.exports = (robot) ->
  ...
  robot.respond /remind (["'\w:\-_]+) in ((?:(?:\d+) ?(?:weeks?|days?|hours?|hrs?|minutes?|mins?|seconds?|secs?|s?)[ ,]*(?:and)? +)+)to (.*)/i, (msg) ->
    who = msg.match[1]
    time = msg.match[2]
    action = msg.match[3]
    if who != "me"
      matchedUsers = robot.usersForGroup who
      if matchedUsers.length > 0
        msg.message.user.name = matchedUsers.join(", ")
      else
